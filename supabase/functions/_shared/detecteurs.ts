...
// -------------------------------
// Ce module a vécu deux mois dans `src/detecteurs.ts` — écrit, testé, et
// sans un seul importateur. Du code payé qui ne produisait rien : l'étage
// déterministe est GRATUIT, mais il ne tournait nulle part, parce que le
// seul endroit où il a du sens est le serveur d'ingestion, qui voit passer
// chaque message une fois et une seule.
//
// Il doit donc tourner dans TROIS runtimes : Deno (l'Edge Function
// d'ingestion), le navigateur (Vite) et Node (le test). Deno exige une
// extension `.ts` explicite à l'import, TypeScript en
// `moduleResolution: bundler` la refuse : un module qui importe quoi que
// ce soit ne peut pas être partagé tel quel. D'où la contrainte que ce
// fichier s'impose et que `scripts/test-detecteurs.cjs` vérifie : AUCUN
// import, aucune API de plateforme. C'est exactement le régime de
// `_shared/rattachement.ts`, dont ce module reprend la méthode.
//
// Les trois valeurs qu'il ne peut donc pas importer — `fold`, `addDays` et
// le lexique d'action — sont traitées différemment selon leur nature :
// les deux fonctions sont des COPIES, et le test compare les deux
// implémentations sur une batterie d'entrées (leur comportement, pas leur
// texte) ; le lexique, lui, a son AUTORITÉ ICI, et `src/tagging.ts`
// l'importe. Une seconde liste de marqueurs aurait fini par diverger, et
// la divergence se verrait là où on la remarque le moins.
//
...
// ------------------------------------------------------------
// 0. Ce que le module ne peut pas importer
// ------------------------------------------------------------
...
/** 'AAAA-MM-JJ' + n jours. Copie EXACTE de `addDays` (src/util.ts), midi
 *  compris : parser à minuit ferait basculer le jour d'un cran selon le
 *  fuseau du runtime — et une échéance décalée d'un jour reste plausible,
 *  donc invisible. Le test compare les deux implémentations. */
...
/**
 * LE lexique d'action, dont c'est ici l'autorité (`src/tagging.ts`
 * l'importe). Il sert à deux questions différentes — « cette note
 * ressemble-t-elle à une action ? » pour les tags, « cette phrase de
 * message propose-t-elle une tâche ? » ici — et c'est justement pourquoi
 * il ne doit exister qu'une fois : deux listes se seraient enrichies
 * séparément, et un marqueur ajouté d'un côté aurait manqué de l'autre
...
// ------------------------------------------------------------
// 0 bis. Les formes de charge utile
// ------------------------------------------------------------
//
// Les clés sont celles de la BASE, contrainte en SQL genre par genre
// (`propositions_charge_utile_chk`). Elles sont redéclarées ici parce que
// le module n'importe rien ; `src/detecteurs.ts` vérifie au TYPE, donc à
// la compilation, qu'elles restent assignables à celles de
// `src/propositions.ts`. Une divergence d'orthographe ne se verrait
// autrement qu'à l'insertion, côté serveur, sur une détection qu'on ne
// reverra pas.
...
export function detecter(texte: string, envoyeLe: string | null): DetectionBrute[] {
  ...
  for (const phrase of phrases(texte)) {
    ...
    // --- tâches : un marqueur d'action, celui du lexique ci-dessus --------
...
// ------------------------------------------------------------
// 6. L'empreinte : rejouer un message ne crée pas de doublon
// ------------------------------------------------------------
//
// L'index unique `(communication_id, genre, empreinte)` de la table
// `propositions` (A.9) est partiel — il ne s'applique qu'aux lignes qui
// PORTENT une empreinte. C'est donc ici que l'idempotence se gagne ou se
// perd : sans empreinte stable, chaque passage du cron (toutes les dix
// minutes) redéposerait les mêmes détections, et la file de revue
// deviendrait illisible en une journée.
//
// L'empreinte porte sur ce qui IDENTIFIE la détection — le genre et
// l'extrait cité — et non sur ce qui peut légitimement bouger d'une
// version du lexique à l'autre : la confiance et les raisons. Y inclure
// la confiance ferait réapparaître toute la file au premier ajustement
// d'un seuil, ce qui est exactement le bruit qu'on veut éviter.
//
// Pas de `crypto.subtle` ici : il est asynchrone, et surtout absent d'un
// des trois runtimes selon les versions. Un hachage textuel court suffit —
// il ne protège rien, il déduplique.
export function empreinteDetection(d: Pick<DetectionBrute, 'genre' | 'extrait'>): string {
  const source = `${d.genre}|${d.extrait}`
  // FNV-1a 32 bits, en base 36 : court, stable, sans dépendance.
  let h = 0x811c9dc5
  for (let i = 0; i < source.length; i++) {
    h ^= source.charCodeAt(i)
    h = Math.imul(h, 0x01000193) >>> 0
  }
  return `lx-${h.toString(36)}-${source.length.toString(36)}`
}
